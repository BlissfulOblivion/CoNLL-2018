...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs): # Forward propogation
        embedded = self.embedding(input).view(1, 1, -1) # Reshapes embedding tensor
        ...
        #    What is the 1 being concatenated? And why dim 1? (figure out later)
        ...
        # ??? Concatenates embedding tensors and attention, but why 0 index? And again, what is the 1?
        ...
        output, hidden = self.gru(output, hidden) # Runs output and hidden information through GRU
...
def train(input_tensor, context_tensor, target_tensor, encoder, decoder, encoder_optimizer,          decoder_optimizer, max_length, criterion):
    ...
    decoder_hidden = torch.cat((decoder_hidden, context_tensor.unsqueeze(2).to(torch.float)), 1) # HELP
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            ...
            decoder_output, decoder_hidden, decoder_attention = decoder(
                decoder_input, decoder_hidden, encoder_outputs)
            topv, topi = decoder_output.topk(1) # ??? Ask Adam or Sarah later
...
def evaluate(encoder, decoder, lemma, context, max_length):
    with torch.no_grad(): # Keeps it from training
        ...
        for di in range(max_length): # Decoder input
            ...
            decoder_output, decoder_hidden, decoder_attention = decoder(
                decoder_input, decoder_hidden, encoder_outputs)
            decoder_attentions[di] = decoder_attention.data # ??? .data?
...
def readLangs(lang1, setting, reverse=False):
    ...
    lines = open('%s_%s.txt' % (lang1, setting), encoding='utf-8').        read().strip().split('\n')
